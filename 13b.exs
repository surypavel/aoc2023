...
defmodule AOC do
  ...
  def find_reflection(items, transposed) do
    ...
    init_range = 1..(len - 1) |> Enum.map(fn index -> {index, 1} end)

    refl =
      items
      |> Enum.reduce_while(init_range, fn item, range ->
        new_range = range |> Enum.map(fn {index, c} ->
          reflects = AOC.reflects?(item, index)
          {index, if reflects do c else c-1 end}
        end) |> Enum.filter(fn { _, c } -> c >= 0 end)
